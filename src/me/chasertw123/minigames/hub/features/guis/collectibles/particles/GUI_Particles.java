package me.chasertw123.minigames.hub.features.guis.collectibles.particles;
 ...
 */
public class GUI_Particles extends AbstractGui {
    ...
    public GUI_Particles(User user) {
        super(6, "Particles", user.getCoreUser());

        cItemStack item = new cItemStack(Material.BARRIER);

        item.setDisplayName(ChatColor.GREEN + "No Particles");
        item.addFancyLore("Tired of things being in your face? Disable your active particle with this.", ChatColor.GRAY.toString());
        ...
        setItem(item, 10, (s, c, p) -> {
            ...
            user.getCoreUser().sendMessage(ChatColor.GREEN + "Disabled your particles!");
        });

        int slot = 0;
        for (Class<? extends Collectible> clazz : me.chasertw123.minigames.core.Main.getCollectibleManager().getCollectiblesByType(CollectibleType.PARTICLE)) {

            ParticleCollectible collectible;
            try {
                collectible = (ParticleCollectible) clazz.getConstructor(me.chasertw123.minigames.core.user.User.class).newInstance(user.getCoreUser());
            } catch (Exception e) {
                e.printStackTrace();
                continue;
            }

            setItem(craftParticleItem(collectible, user), ITEM_SLOTS[slot++], (s, c, p) -> {
                ...
                if (owns) {
                    ...
                    user.activateParticle(collectible);
                    user.getPlayer().sendMessage(ChatColor.GREEN + "Activated Particles: " + collectible.getRarity().getColor() + collectible.getDisplay());
                }
                    ...
                    user.activateParticle(collectible);
                    user.getCoreUser().sendMessage(ChatColor.GREEN + "You unlocked and activated particles: " + collectible.getRarity().getColor() + collectible.getDisplay());
    ...
    private ItemStack craftParticleItem(ParticleCollectible collectible, User user) {
